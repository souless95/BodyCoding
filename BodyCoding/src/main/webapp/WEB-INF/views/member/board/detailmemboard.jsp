...
<body>
   ...
   <div class="container">
      <div style="margin-left: 200px;">
         ...
         <form name="writeFrm">
            ...
            <div class="card-body" style="width: 100%; margin-top: 20px;">
               <table
                  ...
                  <tr style="border-bottom: 1px solid gray;">
                     ...
                     <td colspan="4"
                        ...
                        ${fn:replace(dto.board_contents, replaceChar, "<br/>")} <c:set
                           var="board_file" value="${not empty dto.board_file}" /> <c:if
                           test="${board_file}">
                           <div>
               ...
               <div style="display: flex;">
                  <c:if test="${dto.mem_id eq mem_id}">
                     <button type="button" class="custom-btn btn-1"
                        style="margin: 0px; margin-top: 10px; margin-right: 3px; width: 70px; border: 2px solid #99DAEA;"
                        onclick="location.href='updateboard.do?board_idx=${dto.board_idx }'">
                        수정</button>
                     <button type="button" class="custom-btn btn-1"
                        style="margin: 0px; margin-top: 10px; margin-right: 3px; width: 70px;"
                  ...
                  <div style="margin-top: 10px;">
                     <button type="button" class="custom-btn btn-1"
                        ...
                        style="margin: 0px; width: 70px; margin-right: 0px;">리스트</button>
                     ...
                     <c:if test="${dto.board_category eq '자유' }">
                        <button type="button" style="margin: 0px; width: 90px;"
               ...
               <div>
                  ...
                  <div class="card"
                     style="border: 1px solid #D7D7D7; border-radius: 7px; width: 90%; padding-bottom: 20px;">
                     ...
                     <div class="last-comment" style="margin-left: 15px; width: 100%;">
                        <c:forEach var="rdto" items="${rdto}">
                           <div
                              ...
                              <div
                                 style="display: flex; width: 96%; height: 40px; padding-bottom: 30px; border-bottom: 1px solid #D7D7D7; align-items: center;">
                                 <div>
                                    <div
                                       style="font-weight: bold; width: 220px; height: 20px; display: flex-grow: 0; align-items: center;">${rdto.mem_name}</div>
                                 </div>
                                 &nbsp;
                                 ...
                                 <c:if test="${rdto.mem_id eq mdto.mem_id}">
                                    ...
                                    <div>
                                       <button type="button"
                                          style="margin-bottom: 40px; width: 60px; height: 30px; margin: 2px; display: inline-block; margin-top: 40px;"
                                          class="custom-btn btn-1"
                                          onclick="setUpdateForm(${rdto.reply_idx}, '${rdto.reply_cont}');">수정</button>
                                    </div>
                                    <div>
                                       <button type="button"
                                          style="width: 60px; height: 30px; margin: 2px; display: inline-block; margin-top: 40px;"
                                          class="custom-btn btn-1 "
         ...
         <div style="margin-top: 10px;">
            <form method="post" action="/updatereply.do" class="update-form"
               style="display: none;">
               <input type="hidden" name="board_idx" value="${dto.board_idx }" />
               <input type="hidden" name="mem_id" value="${mdto.mem_id }" /> <input
                  type="hidden" id="reply_idx" name="reply_idx" />
               <div>
                  <div style="width: 90%">
                     <input type="text" name="reply_cont" id="reply_cont"
                        style="width: 90%; height: 50px; border-radius: 30px; margin: 2px;">
                  ...
                  <div>
                     <input type="submit" value="등록" class="custom-btn btn-1"
                        style="margin: 2px;" id="reply_submit_button"
                        onclick="submitUpdatedReply();" />
                     <!-- 수정된 코드 -->
                     <input type="button" value="취소" class="custom-btn btn-1"
                        style="margin: 2px;" onclick="cancelUpdate();" />
                     <!-- 추가된 코드 -->
                  </div>
               </div>
            </form>
         ...
         <div
            style="width: 90%; margin-left: 15px; margin-bottom: 20px; margin-top: -10px;">
            <form method="post" action="/insertreply.do">
               <input type="hidden" name="board_idx" value="${dto.board_idx }" />
               <input type="hidden" name="mem_id" value="${mem_id }" /> <input
                  type="hidden" name="mem_name" value="${mdto.mem_name }" />
               <div
                  style="display: flex; flex-direction: column; width: 88%; margin-top: 10px;">
                  <div>
                     <span style="font-weight: bold;">${mdto.mem_name }</span>
                  </div>
                  <div style="display: flex; width: 100%; margin-left: 20px;">
                     ...
                     <button type="submit" class="custom-btn btn-1"
                        style="width: 65px; margin-left: 5px; margin-right: 0px; margin-bottom: 0px; margin-top: 0px;">등록</button>
                  </div>
               </div>
            </form>
         </div>
