...
.chat_ui .chating .me .b {
	...
	width: 8px;
	height: 22px;
	background: url(/static/uploads/img/chat_balloon_yellow.right.png)
		no-repeat;
	z-index: 1;
}
...
.chat_ui .chating .others .box .a {
	...
	width: 8px;
	height: 22px;
	background: url(/static/uploads/img/chat_balloon_white.left.png)
		no-repeat;
	z-index: 1;
}
				...
				if (msg != null && msg.trim() != '') {
					...
					if (d.type == "getId") {
						...
						if (d.sessionId == $("#sessionId").val()) {
							...
							$("#chating")
									.append(
											"<div class='others' style='margin-top:10px;'>"
													+ "<div class=\"box\">"
	...
	<div class="chat_ui" id="chat_ui" style="width: 320px; height: 557px;">
				...
				<c:choose>
					<c:when test="${c.send_id eq memberid}">
						<div class="me" style="margin-top: 10px;">
							...
							<div class="a" style="padding: 6px 8px 0px 5px;">
								${c.content }</div>
							<div class="time">${c.regidate }</div>
						</div>
					</c:when>
					<c:otherwise>
						<div class="others" style="margin-top: 10px;">
							<div class="box">
								<div class="profile_name">${c.send_id}</div>
								<div class="a"></div>
		...
		<div id="yourName">
			<table class="inputTable">
				<tr>
					<th>사용자명</th>
					<th><input class="textarea1" type="text" name="mem_id"
						id="mem_id" value="${memberid }"></th>
					<th><button class="button" id="reginame" onclick="chatName()">이름
							등록</button></th>
				</tr>
			</table>
		</div>
		<div id="yourMsg">
			<div class="inputTable">
				...
				<button class="button" id="sendBtn">전송</button>
				<div class="clear"></div>
			</div>
		</div>
	</div>
